...
run_dir_name = datetime.now().strftime("%Y%m%d_%H%M%S") + f"_alexnet_b{BATCH_SIZE}_sgd{LEARNING_RATE}_wd{WEIGHT_DECAY}"

os.makedirs(run_save_dir, exist_ok=True)


def create_rgb_alexnet(num_classes=2):
...
for i in range(1, 9):  # Iterate across 8 EEG channels
    print(f"\n--- Training AlexNet for Channel {i} (SGD LR={LEARNING_RATE}, WD={WEIGHT_DECAY}) ---")
    ...
    model = create_rgb_alexnet(num_classes=NUM_CLASSES)
    ...
    best_model_path = os.path.join(run_save_dir, f'best_alexnet_ch{i}_rgb_sgd.pth')
