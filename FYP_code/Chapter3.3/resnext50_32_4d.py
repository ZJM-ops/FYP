...
run_dir_name = datetime.now().strftime("%Y%m%d_%H%M%S") + f"_resnext50_b{BATCH_SIZE}_sgd{LEARNING_RATE}_wd{WEIGHT_DECAY}"
...
def create_resnext50_32x4d(num_classes=2, pretrained=False):
...
for i in range(1, 9): 
    print(f"\n--- Training ResNeXt-50 for Channel {i} (SGD LR={LEARNING_RATE}, WD={WEIGHT_DECAY}) ---")
    ...
    model = create_resnext50_32x4d(num_classes=NUM_CLASSES, pretrained=False)
    optimizer = optim.SGD(
        model.parameters(), 
        lr=LEARNING_RATE, 
        momentum=0.9, 
    ...
    best_model_path = os.path.join(run_save_dir, f'best_resnext50_32x4d_ch{i}_sgd.pth')
    ...
    if os.path.exists(best_model_path):
        ...
        print(f"Warning: Best model not found at {best_model_path}, using weights from last epoch.")
