...
run_dir_name = datetime.now().strftime("%Y%m%d_%H%M%S") + f"_resnet18_b{BATCH_SIZE}_sgd{LEARNING_RATE}_wd{WEIGHT_DECAY}"
...
# ResNet18
def create_rgb_resnet18(num_classes=2):
...
for i in range(1, 9): # 8个通道
    print(f"\n--- Training ResNet-18 for Channel {i} (SGD LR={LEARNING_RATE}, WD={WEIGHT_DECAY}) ---")
    ...
    model = create_rgb_resnet18(num_classes=NUM_CLASSES)
    ...
    best_model_path = os.path.join(run_save_dir, f'best_resnet18_ch{i}_rgb_sgd.pth')
    ...
    if os.path.exists(best_model_path):
        ...
        print(f"Warning: Best model file not found at {best_model_path}. Using current model weights.")
