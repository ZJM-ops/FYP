...
run_dir_name = datetime.now().strftime("%Y%m%d_%H%M%S") + f"_resnet50_b{BATCH_SIZE}_sgd{LEARNING_RATE}_wd{WEIGHT_DECAY}_nopretrain"
run_save_dir = os.path.join(model_save_dir, run_dir_name)

os.makedirs(run_save_dir, exist_ok=True)
print(f"Models and logs will be saved to: {run_save_dir}")
...
def create_rgb_resnet50(num_classes=2, pretrained=False):
    if pretrained:
...
for i in range(1, 9): # 8个通道
    print(f"\n--- Training ResNet-50 (No Pretrain) for Channel {i} (SGD LR={LEARNING_RATE}, WD={WEIGHT_DECAY}) ---")
    ...
    model = create_rgb_resnet50(num_classes=NUM_CLASSES, pretrained=False)
    ...
    best_model_path = os.path.join(run_save_dir, f'best_resnet50_ch{i}_rgb_sgd_nopretrain.pth')
    ...
    for epoch in range(EPOCHS):
        ...
        if val_loss < best_val_loss:
            ...
            dir_to_save = os.path.dirname(best_model_path)
    ...
    if os.path.exists(best_model_path):
        ...
        print(f"Warning: Best model not found at {best_model_path}, using last epoch weights.")
