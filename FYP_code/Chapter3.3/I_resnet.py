...
run_dir_name = datetime.now().strftime("%Y%m%d_%H%M%S") + f"_inception_resnet_v2_b{BATCH_SIZE}_sgd{LEARNING_RATE}_wd{WEIGHT_DECAY}"
...
def create_inception_resnet_v2(num_classes=2, pretrained=False):
...
for i in range(1, 9): # 8个通道
    print(f"\n--- Training Inception-ResNet-v2 for Channel {i} (SGD LR={LEARNING_RATE}, WD={WEIGHT_DECAY}) ---")
    ...
    model = create_inception_resnet_v2(num_classes=NUM_CLASSES)
    ...
    best_model_path = os.path.join(run_save_dir, f'best_inception_resnet_v2_ch{i}_sgd.pth')
    ...
    for epoch in range(EPOCHS):
        ...
        if val_loss < best_val_loss:
            ...
            current_save_dir = os.path.dirname(best_model_path)
    ...
    if os.path.exists(best_model_path):
        ...
        print(f"Warning: Best model not found at {best_model_path}. Using current model weights.")
