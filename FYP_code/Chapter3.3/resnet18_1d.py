...
class ResNet18_1D(nn.Module):
    def __init__(self, num_classes=2, in_channels=8):
        ...
        # Layers: [2, 2, 2, 2] blocks for ResNet18
...
run_dir_name = datetime.now().strftime("%Y%m%d_%H%M%S") + f"_resnet18_1d_b{BATCH_SIZE}_sgd"
...
model = ResNet18_1D(num_classes=NUM_CLASSES, in_channels=INPUT_CHANNELS).to(DEVICE)
...
best_model_path = os.path.join(run_save_dir, 'best_resnet18_1d.pth')

print("\n--- Starting Training (ResNet-18 1D) ---")
